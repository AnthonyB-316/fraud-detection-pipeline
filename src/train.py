...
from features import engineer_features, load_data, prepare_train_test
...
def plot_metrics(model, X_test, y_test, save_path="models/metrics.png"):
    ...
    axes[0].set_title(
        f"Precision-Recall Curve (AUC={average_precision_score(y_test, y_proba):.3f})"
    )
...
def main(use_mlflow=True, target_recall=0.94):
    ...
    with mlflow.start_run() if use_mlflow else nullcontext():
        ...
        optimal_threshold = find_optimal_threshold(
            model, X_test, y_test, target_recall=target_recall
        )
        ...
        if use_mlflow:
            mlflow.xgboost.log_model(
                model, "model", registered_model_name="fraud-detection-xgboost"
            )
