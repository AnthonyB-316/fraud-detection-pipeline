...
def prepare_train_test(df: pd.DataFrame, test_size: float = 0.2, random_state: int = 42):
    ...
    print(f"Before SMOTE - Train: {len(X_train):,} | Fraud: {y_train.sum():,} ({y_train.mean():.4%})")

    smote = SMOTE(random_state=random_state, sampling_strategy=0.5)
    X_train_resampled, y_train_resampled = smote.fit_resample(X_train, y_train)

    print(f"After SMOTE - Train: {len(X_train_resampled):,} | Fraud: {y_train_resampled.sum():,} ({y_train_resampled.mean():.4%})")
